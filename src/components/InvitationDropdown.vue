...
<template>
    <div class="relative">
        <button @click="isOpen = !isOpen"
            class="relative p-2 text-gray-400 hover:text-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-800">
            ...
            <span v-if="invitations.length > 0"
                class="absolute top-0 right-0 block h-2 w-2 rounded-full bg-green-500 ring-2 ring-white" />
        </button>
                ...
                <div v-for="invitation in invitations" :key="invitation.id"
                    ...
                    <div class="flex flex-col space-y-1">
                        <p class="text-sm font-medium text-gray-900 break-words flex items-center gap-2">
                            <svg class="h-4 w-4 text-green-600 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="flex flex-col sm:flex-row gap-2 mt-2">
                            <button @click="handleResponse(invitation.id, true)"
                                class="w-full bg-green-600 text-white px-3 py-1 rounded-md text-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center justify-center gap-1">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M5 13l4 4L19 7" />
                                </svg>
                            ...
                            <button @click="handleResponse(invitation.id, false)"
                                class="w-full bg-white text-gray-700 px-3 py-1 border rounded-md text-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center justify-center gap-1">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
